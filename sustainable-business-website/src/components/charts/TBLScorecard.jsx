...
function SpringBar({ value, color, delay }) {
  ...
  return (
    <div ref={ref} style={{ height: '14px', background: '#d8e0ea', border: '1px solid #b8c4d4', borderRadius: '3px', overflow: 'hidden' }}>
      <motion.div
        initial={{ width: 0 }}
...
export default function TBLScorecard() {
  ...
  let idx = 0
  return (
    <div className="chart" style={{ padding: '8px 10px' }}>
      <div className="chart__title">Triple Bottom Line Scorecard</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', width: '100%', flex: 1 }}>
        {PILLARS.map((pillar, pi) => (
          <div key={pi} style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <motion.div
              ...
              transition={{ delay: pi * 0.12, duration: 0.3 }}
              style={{
                fontSize: '13px', color: pillar.color,
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              {pillar.metrics.map((m, mi) => {
                ...
                const delay = pi * 0.12 + mi * 0.08
                return (
                  <motion.div key={mi}
                    initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: delay + 0.1 }}
                    onMouseEnter={() => setHovered(i)} onMouseLeave={() => setHovered(null)}
                    style={{ flex: 1, cursor: 'default', position: 'relative' }}
                  >
                    <div style={{ fontSize: '10px', color: '#446', fontFamily: 'var(--font-mono)', marginBottom: '4px', lineHeight: 1.3 }}>{m.label}</div>
                    <SpringBar value={m.value} color={barColor} delay={delay} />
                    <div style={{ fontSize: '11px', color: barColor, fontFamily: 'var(--font-mono)', marginTop: '3px', fontWeight: 700 }}>{m.display}</div>
                    {hovered === i && (
                      <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }}
                        style={{
                          position: 'absolute', bottom: '100%', left: 0, zIndex: 20,
                          fontSize: '10px', color: '#001e50',
                          fontFamily: 'var(--font-mono)', background: '#ffffff',
                          border: `1px solid ${barColor}`, padding: '5px 7px',
                          marginBottom: '4px', lineHeight: 1.5, maxWidth: '180px',
                          whiteSpace: 'normal', boxShadow: '0 4px 12px rgba(0,30,80,0.15)',
                        }}
